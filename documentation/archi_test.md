...
- **2026-09-13** — Vérification demandée par le plan de tests, section [Ce que ces tests couvrent indirectement](plan_de_tests.md#ce-que-ces-tests-couvrent-indirectement) : `JwtAuthenticationFilter` est bien couvert (100 % lignes, 62 % branches) grâce aux scénarios avec/sans token de `StudentControllerTest`. `RestExceptionHandler` plafonne à 53 % — les handlers `BadCredentialsException` et `AccessDeniedException` ne sont couverts par aucun test car rien dans le code actuel ne lève ces exceptions (le login utilise `IllegalArgumentException`, et il n'y a pas d'autorisation par rôle) : code mort du point de vue des tests, pas un manque de test à combler. La couverture globale (90,2 %) reste largement au-dessus du seuil de 80 %.
